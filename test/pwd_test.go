...
func TestTools(t *testing.T) {
	Convey("Gene password", t, func() {
		...
		for _, val := range pass {
			if strings.Index(opt.Up, string(val)) > -1 {
				res.Up++
				continue;
			}

			if strings.Index(opt.Down, string(val)) > -1 {
				res.Down++
				continue;
			}

			if strings.Index(opt.Digits, string(val)) > -1 {
				res.Digit++
				continue;
			}

			if strings.Index(opt.Specials, string(val)) > -1 {
				res.Special++
				continue;
			}
		}
